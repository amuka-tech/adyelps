...
export default async function Home({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  // 2. Fetch News and Events in parallel
  const [
    { data: newsData },
    { data: eventsData }
  ] = await Promise.all([
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {displayItems.map((item, idx) => (
              <Card key={idx} className="h-full flex flex-col group overflow-hidden border-gray-100 shadow-sm hover:shadow-lg transition-all bg-white">
                ...
                <div className="flex-1 flex flex-col p-6">
                  ...
                  <Link href={`/${item.type === 'News' ? 'news' : 'events'}/${item.id}`} className="text-maroon font-bold text-sm flex items-center hover:text-pink transition-colors w-fit mt-auto">
                    View Details 
